...
with open('./{}/ATOMIC_tails_test.pkl'.format(path_dir), 'rb') as f:
    arr_tails_test = pickle.load(f)

with open('./{}/ATOMIC_tails_train.pkl'.format(path_dir), 'rb') as f:
    arr_tails_train = pickle.load(f)

with open('./{}/ATOMIC_tails_valid.pkl'.format(path_dir), 'rb') as f:
    arr_tails_valid = pickle.load(f)

dicts = defaultdict(list)
ALL = []
ALL_train_test = []
for line in open('../preprocess/ATOMIC-Ind-test.txt'):
    info = line.strip().split('\t')
    ALL.append(info)
    ALL_train_test.append(info)
    dicts[info[0]].append(info[1])

for line in open('../preprocess/ATOMIC-Ind-train.txt'):
    info = line.strip().split('\t')
    ALL_train_test.append(info)
    dicts[info[0]].append(info[1])

for line in open('../preprocess/ATOMIC-Ind-valid.txt'):
    info = line.strip().split('\t')
    ALL_train_test.append(info)
    dicts[info[0]].append(info[1])
...
for ind_row in range(0, arr_heads_test.shape[0], 5000):
    ...
    for ind, (info, score) in enumerate(zip(ALL[ind_row:min(ind_row+5000, arr_heads_test.shape[0])], scores)):
        ...
        # same tail has same score, then same rank, same label
